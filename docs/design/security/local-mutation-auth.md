...
- `screenshot` / `screenshot/interactive` 走 `_is_loopback_request` 拒绝非环回请求（`main_routers/system_router.py:194`）
- `proactive_chat` 用 `mgr.state.can_start_proactive(...)` / `try_start_proactive(...)` 做并发 / 状态机门控（`main_routers/system_router.py:4580` / `4605`），并发拒绝时返回 409
...
> **关于命名不对称**（`NEKO_AUTOSTART_CSRF_TOKEN` 单键 vs `AUTOSTART_ALLOWED_ORIGINS` 双键）：实现历史差异——CSRF token 在 `config/__init__.py:284` 走 `os.getenv()` 直读，allowed origins 走 `_read_list_env()` helper。统一化（让 token 也走 helper 拿到双键 fallback）不在 issue #1479 范围，没有 known 痛点；如果以后运维真的因为这个命名差异踩坑，再单独 PR。**实务建议：所有 N.E.K.O. 相关 env 都加 `NEKO_` 前缀，这套规则在所有 helper 下都生效**。
...
3. **临时止血**：在用户机器的 N.E.K.O. 源码 `main_routers/system_router.py` 顶部 `_validate_local_mutation_request` 第一行加 `return None`，重启服务（关掉守卫，**只是临时操作**，绝不要 commit 到 main）
...
- **`/api/personal_dynamics` dead-code 清理**：grep 全仓没有调用方，后端内部直接调函数。要么删 endpoint，要么改成 GET。issue #1479 不在范围，单独跟进。
