...
(function(global) {
    ...
    class Driver {
        ...
        createStyles() {
            ...
            style.textContent = `
                ...
                .driver-highlight {
                    ...
                    box-shadow: 0 0 30px rgba(68, 183, 254, 0.8), inset 0 0 30px rgba(255, 255, 255, 0.15) !important;
        ...
        createHighlight(element) {
            ...
            // 创建 SVG 遮罩层（evenodd 挖洞，替代 box-shadow 方案）
            const ns = 'http://www.w3.org/2000/svg';
            this.overlay = document.createElementNS(ns, 'svg');
            this.overlay.setAttribute('class', 'driver-overlay');
            this.overlay.setAttribute('xmlns', ns);
            this.overlay.style.pointerEvents = 'none'; // 明确禁止，不依赖 CSS，防止 CEF 拦截点击

            this._overlayPath = document.createElementNS(ns, 'path');
            this._overlayPath.setAttribute('fill', 'rgba(0,0,0,0.6)');
            this._overlayPath.setAttribute('fill-rule', 'evenodd');
            this._overlayPath.setAttribute('d', this._buildMaskPath(x, y, w, h, vw, vh, r));
            this.overlay.appendChild(this._overlayPath);

            document.body.appendChild(this.overlay);

            // 高亮框：只保留边框发光，不再用 box-shadow 做遮罩
            ...
            console.log('[Driver] Highlight created at:', { top: y, left: x, width: w, height: h });
        }

        _buildMaskPath(x, y, w, h, vw, vh, r) {
            // 全屏矩形
            const full = `M 0 0 H ${vw} V ${vh} H 0 Z`;
            // 高亮圆角矩形（evenodd 规则：两个路径区域奇偶叠加，重叠部分镂空）
        ...
        positionPopover(element) {
            ...
            const vw = window.innerWidth;
            const vh = window.innerHeight;
            const popoverWidth = this.popover.offsetWidth || 380;
            ...
            const clamp = (left, top) => {
                ...
                if (l + popoverWidth > vw - margin) l = Math.max(margin, vw - popoverWidth - margin);
                if (t < margin) t = margin;
                if (t + popoverHeight > vh - margin) t = Math.max(margin, vh - popoverHeight - margin);
                return { left: l, top: t };
            };

            const boxAt = (left, top) => {
                const c = clamp(left, top);
                return {
                    left: c.left,
                    top: c.top,
                    right: c.left + popoverWidth,
                    bottom: c.top + popoverHeight
                };
            };

            if (element) {
                const rect = element.getBoundingClientRect();
                const wideTarget = rect.width > vw * 0.45;
                ...
                // 宽控件（如全宽下拉）优先放在下方，避免左右钳位后仍压住目标
                const candidates = wideTarget
                    ? [belowLeft, belowCenter, rightOf, leftOf, above]
                    : [rightOf, leftOf, belowLeft, belowCenter, above];

                let chosen = null;
                for (const gen of candidates) {
                    const p = gen();
                    const b = boxAt(p.left, p.top);
                    if (!intersects(b, rect)) {
                        chosen = { left: b.left, top: b.top };
                        break;
                    }
                }
                ...
                if (!chosen) {
                    ...
                    let l = rect.left + (rect.width - popoverWidth) / 2;
                    let b = boxAt(l, t);
                    chosen = { left: b.left, top: b.top };
                    let n = 0;
                    while (intersects({ left: chosen.left, top: chosen.top, right: chosen.left + popoverWidth, bottom: chosen.top + popoverHeight }, rect) && n < 40) {
                        t += 14;
                        b = boxAt(l, t);
                        chosen = { left: b.left, top: b.top };
                        n++;
                ...
                this.popover.style.left = chosen.left + 'px';
                this.popover.style.top = chosen.top + 'px';
            } else {
                let top = (vh - popoverHeight) / 2;
                let left = (vw - popoverWidth) / 2;
        ...
        destroy() {
            ...
            if (this.dragHandlers) {
                this.dragHandlers.forEach(({ handleDragMove, handleDragEnd }) => {
        ...
        refresh() {
            ...
            if (this._overlayPath) {
                const vw = window.innerWidth;
                const vh = window.innerHeight;
                const newX = rect.left - padding;
                const newY = rect.top - padding;
                const newW = rect.width + padding * 2;
                const newH = rect.height + padding * 2;
                this._overlayPath.setAttribute('d', this._buildMaskPath(newX, newY, newW, newH, vw, vh, 8));
            }
